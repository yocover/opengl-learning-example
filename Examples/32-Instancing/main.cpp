...
int main()
{
	...
	Shader shader("model_vert.glsl", "model_frag.glsl");
	Shader instanceShader("instance_model_vert.glsl", "instance_model_frag.glsl");
	...
	for (unsigned int i = 0; i < amount; i++) {
		...
		// 1.位移 : 分布在半径为 radius 的圆形上，偏移的范围 [-offset, offset]
		float angle = (float)i / (float)amount * 360.0f;
		float displacement = (rand() % (int)(2 * offset * 100)) / 100.0f - offset;
		float x = sin(angle) * radius + displacement;
		displacement = (rand() % (int)(2 * offset * 100)) / 100.0f - offset;
		float y = displacement * 0.4f; // 让行星带的高度比x和z都要小
		displacement = (rand() % (int)(2 * offset * 100)) / 100.0f - offset;
		float z = cos(angle) * radius + displacement;
		model = glm::translate(model, glm::vec3(x, y, z));

		// 2. 缩放 : 在 0.05f 和 0.25f 之间缩放
		float scale = (rand() % 20) / 100.0f + 0.05;
		model = glm::scale(model, glm::vec3(scale));

		// 3. 旋转 : 绕着一个随机选择的旋转轴向量进行随机的旋转
